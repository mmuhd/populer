            ...
            <li class="hover:bg-gray-100 first:rounded-t-md last:rounded-b-md flex bg-white">
                <a class="flex-auto p-3 no-underline flex space-x-4" href="/app/{{$top->slug}}">
                    <div class="sm:w-20 sm:h-20 w-12 h-12 flex-shrink-0">
                        <img class="w-full max-h-full object-contain" src="{{asset('storage/' .$top->thumbnail) }}" alt="topic thumnail">
                    </div>
                    <div class="flex flex-col">
                        <h2 class="font-bold" data-score="0.23645480586078318">{{$top->name}}</h2>
                        <div class="text-sm flex-auto">{{$top->one_liner}}</div>
                        <div class="sm:flex-row sm:space-y-0 sm:space-x-4 flex flex-col mt-2 space-y-1">
                            ...
                            <div class="flex space-x-4">
                                <div class="text-xs text-gray-600">{{$top->created_at->diffForHumans()}}</div>
                ...
                <div class="p-3">
                    <form action="/app/upvote/{{$top->id}}" method="post" onsubmit="">
                        @csrf
                        <button class="sm:w-16 hover:bg-gray-100  upvote-btn group w-12" type="submit" id="populer_btn" title="">
